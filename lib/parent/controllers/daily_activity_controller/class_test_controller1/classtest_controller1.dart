...
class ClassTestController1 extends BaseController with GetTickerProviderStateMixin {
  DateTime currentDate = DateTime.now();
  bool isExpanded = false;
  String startDate = "";
  String endDate = "";
  int currentTab = 0;
  Color percentageColor = Color(0XFFF9F9F9);
  String? performance;
  late TabController tabController;

  List<ClassTestData> homeworkDataList = [];
  List<ClassTestData> finalList = <ClassTestData>[].obs;
 ...
 /* void exploreView(int index,int index1){
  ...
  void _getData() async {
    ...
    finalList = [];
    List<ClassTestData> listOfData = [];
    if (arguments["tag"] != null) {
      if (arguments["tag"] == "ClassTest") {
        if (currentTab == 0) {
          listOfData = await fetchData(
              url:
                  "${ApiHelper.classTestUrl}student_id=${181}");//${LocalStorage.getValue('studentId')}
          ...
          listOfData = await fetchData(
              url:
                  "${ApiHelper.classTestReportUrl}student_id=${LocalStorage.getValue('studentId')}&start_date=$startingDateValue&end_date=$endingDateValue&type=$type&page=$_pageNo");
        }
      }
      finalList.clear();
  ...
  Future<List<ClassTestData>> fetchData({String? url}) async {
    homeworkDataList = [];
    try {
      final result = await BaseService().getMethod(url ?? "");
      if (result != null) {
        if (result.statusCode == 200) {
          ClassTestExampleModel homeWorkModel = ClassTestExampleModel.fromJson(result.data);
          homeworkDataList = homeWorkModel.classTestData ?? [];
          print("classTest - ${result.statusCode}");
        } else {
          homeworkDataList = [];
          print("classTest - ${result.statusCode}");
        }
