...
class NewsCircularEventController extends BaseController
      ...
      if (arguments["tag"] == "Circular") {
        ...
        if (currentTab == 0) {
          listOfData = await fetchData(
              url:
                  "${ApiHelper.circularUrl}student_id=${LocalStorage.getValue('studentId')}&remove_empty_image=1&page=$_pageNo");
        } else {
          listOfData = await fetchData(
              url:
                  "${ApiHelper.pastUrl}student_id=${LocalStorage.getValue('studentId')}&type=$type&start_date=$startingDateValue&end_date=$endingDateValue&remove_empty_image=1&page=$_pageNo");
        }
        ...
        if (currentTab == 0) {
          listOfData = await fetchData(
              url:
                  "${ApiHelper.eventUrl}student_id=${LocalStorage.getValue('studentId')}&remove_empty_image=1&page=$_pageNo");
        } else {
          listOfData = await fetchData(
              url:
                  "${ApiHelper.pastUrl}student_id=${LocalStorage.getValue('studentId')}&type=$type&start_date=$startingDateValue&end_date=$endingDateValue&remove_empty_image=1&page=$_pageNo");
        ...
        if (currentTab == 0) {
          listOfData = await fetchData(
              url:
                  "${ApiHelper.newsUrl}student_id=${LocalStorage.getValue('studentId')}&remove_empty_image=1&page=$_pageNo");
        } else {
          listOfData = await fetchData(
              url:
                  "${ApiHelper.pastUrl}student_id=${LocalStorage.getValue('studentId')}&type=$type&start_date=$startingDateValue&end_date=$endingDateValue&remove_empty_image=1&page=$_pageNo");
        ...
        if (arguments["tag"] != null) {
          if (arguments["tag"] == "Circular") {
            ...
            if (currentTab == 0) {
              listOfData = await fetchData(
                  url:
                      "l${ApiHelper.eventUrl}student_id=${LocalStorage.getValue('studentId')}&remove_empty_image=1&page=${++_pageNo}");
            } else {
              listOfData = await fetchData(
                  url:
                      "${ApiHelper.pastUrl}student_id=${LocalStorage.getValue('studentId')}&type=$type&start_date=$startingDateValue&end_date=$endingDateValue&remove_empty_image=1&page=${++_pageNo}");
